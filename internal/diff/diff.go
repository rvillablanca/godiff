...
loop:
	...
	if len(toRemove) > 0 {
		fileName := filepath.Join(destDir, getDeleteFilename())
		...
		fmt.Printf("files to be removed have been listed in %s\n", getDeleteFilename())
	}
...
func writeFile(f *os.File, file string) {
	if runtime.GOOS == "windows" {
		_, _ = f.WriteString("del " + file + "\n")
		return
	}
...
func writeHeader(f *os.File) {
	if runtime.GOOS == "windows" {
		_, _ = f.WriteString("REM THIS FILE WAS GENERATED BY godiff\n")
		return
	}
...
func getDeleteFilename() string {
	if runtime.GOOS == "windows" {
		return "to_delete.bat"
	}
